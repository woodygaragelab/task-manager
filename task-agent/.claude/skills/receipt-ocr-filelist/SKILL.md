...
description: |
  ...
  Driveのrenamedフォルダへ保存し、それ以外(売上・給与・銀行通帳)はリネームせず
  そのまま該当フォルダ(売上/給与/銀行通帳)へ保存するスキル(AgentCore版)。対応する
...
`receipt_filelist.xlsx` に追記し、勘定科目名ごとに整理したコピーをDrive上の`renamed`
フォルダへ保存する(従来の処理と同じ)。売上・給与・銀行通帳に分類されたものは、
OCRやリネームは行わず、そのままDrive上の該当フォルダ(`売上`/`給与`/`銀行通帳`)へ
保存する。仕訳データ(弥生会計形式)の作成は別スキル`receipt-to-journal`が担当する
...
- 支払のリネーム済みコピーの保存先: receiptフォルダの親フォルダ直下の `renamed`
  フォルダ(Driveの`parentId`をたどって取得する。ローカル版の「関与先フォルダ」に相当)
- 売上・給与・銀行通帳の保存先: receiptフォルダの親フォルダ直下の `売上` / `給与` /
  `銀行通帳` フォルダ(`renamed`と同じ階層。無ければ新規作成する。リネームはせず
  元のファイル名のまま保存する)
...
`renamed/未分類/` に保存されるため、後から人の目で見つけやすい)。
...
1. receiptフォルダの親フォルダ(=関与先フォルダ)直下に、分類先の名前
   (`売上` / `給与` / `銀行通帳`)のフォルダを `search_files` で探す
   (無ければ`create_file`で`mimeType: application/vnd.google-apps.folder`を
   指定してフォルダを新規作成する。`renamed`フォルダと同じ階層)。
