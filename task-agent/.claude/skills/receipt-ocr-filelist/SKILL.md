...
画像・PDFはOCRとして、Excel/CSVファイル(`.xlsx`/`.xls`/`.csv`)は表データとして
読み取る(支払についても、請求明細等がスキャン画像ではなくExcel/CSV形式で
受領フォルダに置かれる場合があるため。対応拡張子はStep 1(`EXT_TYPE`)で分類の別なく
共通)。売上・給与・銀行通帳に分類したファイルはOCRを行わず、このStepの対象外
(Step 4でファイル名などの機械的な情報だけを記録する)。
...
1. `mcp__task-manager__list_series` を呼び、`taskGroup`(=分類/category)が `"支払"` で
   `seriesName`(=シリーズ名)が `"資料受領"` である項目の `seriesCode` を控える
   (=タスク名(series_code)に使う値。見つからない場合は履歴登録を行わずその旨を報告する)。
2. `mcp__task-manager__list_history(client_code="{関与先コード}")` を呼び、
   `content` が `支払資料の自動分類、リネーム保存` と一致するレコードが既にあるか確認する。
...
- 分類(category): `支払`
- タスク名(series_code): 1.で控えた「資料受領」のseriesCode
- 担当者(assignee): `整理係`
- ステータス(status): `完了`
- 内容(content): `支払資料の自動分類、リネーム保存`
