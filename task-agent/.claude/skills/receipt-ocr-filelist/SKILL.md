...
description: |
  ...
  Driveの整理済フォルダへ保存し、それ以外(売上・給与・銀行通帳)はリネームせず
...
`receipt_filelist.xlsx` に追記し、勘定科目名ごとに整理したコピーをDrive上の`整理済`
...
- 売上・給与・銀行通帳の保存先: `整理済`フォルダ直下の `売上` / `給与` /
  `銀行通帳` フォルダ(`整理済/売上`・`整理済/給与`・`整理済/銀行通帳`。無ければ
  新規作成する。リネームはせず元のファイル名のまま保存する)
...
`整理済/未分類/` に保存されるため、後から人の目で見つけやすい)。
...
`整理済/<勘定科目名>/` フォルダを `search_files` で探す(無ければ`create_file`で
...
1. receiptフォルダの親フォルダ(=関与先フォルダ)直下の `整理済` フォルダの下に、
   ...
   指定して新規作成する。支払のリネーム済みコピー(`整理済/<勘定科目名>/`)と同じ
   `整理済`フォルダの配下)。
