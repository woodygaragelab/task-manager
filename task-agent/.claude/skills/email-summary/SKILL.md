...
name: email-summary
description: |
  Gmailの新着受信メールを要約し、差出人・転送/返信元の差出人・宛先ドメインから
...
- 関与先が指定された場合:Step 0で取得した対応表からその関与先の`senderDomains`を
  引き、Step 1の検索クエリを `label:inbox -label:処理済み (from:*@domain1 OR
  to:*@domain1 OR from:*@domain2 OR ...)` のように絞り込む(`senderDomains`が
  複数ある場合はOR条件で並べる)。ただし転送・返信元の差出人がこのドメインである
  ケース(Step 2の判定2)は件名・本文を読まないと分からないため、直近差出人・宛先が
  他ドメインでも一致する可能性のあるメールを取りこぼすことがある点に注意
...
1. `mcp__task-manager__list_clients` を呼び、`clientCode → {senderDomains, uketoriFolderId}`
...
1. **直近の差出人(From)ドメイン**をStep 0の `senderDomains` 対応表と照合する
2. 一致しなければ、**転送・返信元の差出人ドメイン**を調べる。`bodyText` 中に
   引用された元メールのヘッダー(「From:」「差出人:」「-------- Forwarded
   message --------」「On ... wrote:」等の引用ブロック)がある場合、そこに
   記載された元の差出人メールアドレスのドメインを抽出し、同様に照合する
   (引用が多重(転送の転送等)の場合は、本文中に出てくる差出人ドメインを
   すべて拾って照合する)
3. それでも一致しなければ、**宛先(To/Cc)のメールアドレスドメイン**を
   同じ対応表と照合する
...
- 差出人ドメイン・受領フォルダIDの追加・変更は、taskmanagerのクライアント設定
