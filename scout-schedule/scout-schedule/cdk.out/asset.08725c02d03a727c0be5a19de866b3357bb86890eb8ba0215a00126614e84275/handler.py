...
def handler(event, context):
    ...
    # 一部の関与先の起動に失敗しても例外は投げない。ここで例外を投げると
    # EventBridge Schedulerのretry_policyによりこのhandler全体が再実行され、
    # _start_jobはジョブIDを都度新規UUIDで発行するため、既に起動済みの(=成功した)
    # 関与先分のジョブまでもう一度重複して起動されてしまう(結果としてGmail添付や
    # 履歴が3件などに重複する不具合の原因だった)。失敗した関与先はログの
    # invoke_partial_errorで検知し、次回のスケジュール実行(6時間後)で拾われる
    # (Gmail側に未処理メールが残っている限り、次回実行時に自然に再試行される)。
